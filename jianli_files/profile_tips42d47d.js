...
var _oSelf=this;
var mailContainers=getTop().parents('div.qmbox',_aoTriggerDom);
if(mailContainers&&mailContainers.length>0)
{
return;
}
...
var _sRejectHtmlIdStr='';
_sRejectHtml=_sRejectHtml.replace(/(id=)\"([^\"]+)\"/g,function(_asWhole,_asKey,_asValue){
_sRejectHtmlIdStr=_asValue;
return 'id=""';
});
_sRejectHtml=_sRejectHtml.replace(/(\s[^<\'\"]+?)=([^ \f\n\r\t\v\"\'>]+)/g,function(_asWhole,_asKey,_asValue){
return _asKey+'="'+_asValue+'"';
});
_sRejectHtml=_sRejectHtml.replace(/id=\"\"/g,'id="'+_sRejectHtmlIdStr+'"');
var _sFilingHtmlIdStr='';
_sFilingHtml=_sFilingHtml.replace(/(id=)\"([^\"]+)\"/g,function(_asWhole,_asKey,_asValue){
_sFilingHtmlIdStr=_asValue;
return 'id=""';
});
_sFilingHtml=_sFilingHtml.replace(/(\s[^<]+?)=([^ \f\n\r\t\v\"\'>]+)/g,function(_asWhole,_asKey,_asValue){
return _asKey+'="'+_asValue+'"';
});
_sFilingHtml=_sFilingHtml.replace(/id=\"\"/g,'id="'+_sFilingHtmlIdStr+'"');
